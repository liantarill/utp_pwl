                        ...
                        @forelse ($appointments as $index => $appointment)
                            <tr class="border-b hover:bg-blue-pale transition">
                                ...
                                <td class="px-6 py-3 text-sm font-medium text-gray-800">
                                    {{ $appointment->appointment_number }}</td>
                                <td class="px-6 py-3 text-sm text-gray-700">{{ $appointment->patient->name ?? '-' }}
                                </td>
                                <td class="px-6 py-3 text-sm text-gray-700">{{ $appointment->doctor->user->name ?? '-' }}
                                </td>
                                <td class="px-6 py-3 text-sm text-gray-700">
                                    {{ $appointment->appointment_date->format('d M Y') }}</td>
                                <td class="px-6 py-3 text-sm text-gray-700 capitalize">{{ $appointment->status ?? '-' }}
                                </td>
                                <td class="px-6 py-3 text-center flex justify-center gap-2">
                                    ...
                                    <form action="{{ route('staff.appointments.destroy', $appointment->id) }}"
                                        method="POST" onsubmit="return confirm('Yakin ingin menghapus janji temu ini?')"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
